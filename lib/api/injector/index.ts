...
export class Injector {
   ...
   * @param config
   */
  static register(token: Injectable<any>, config?: { force?: boolean, immediate?: boolean }) {
    if (!this.instance) {
    ...
    if (config && config.immediate) {
      ...
      if (this.instance.injectables.has(token)) {
        throw new Error(`Cannot register token \`${token.name}\` with Injector: token exists. Try using the 'force' option.`);
      }
